...
export function getAllCountries() {
  return async (dispatch) => {
    const countries = await axios.get('https://api-country-kappa.vercel.app/countries')
    return dispatch({
      type: 'GET_COUNTRIES',
  ...
  return async (dispatch) => {
    try {
      const countriesByName = await axios.get(
        `https://api-country-kappa.vercel.app/countries?name=${name}`
      )
      return dispatch({
...
export function getCountryDetails(id) {
  return async (dispatch) => {
    try {
      const details = await axios.get(`https://api-country-kappa.vercel.app/countries/${id}`)
      return dispatch({
...
export function createActivity(details) {
  return async function () {
    const newActivity = await axios.post(
      'https://api-country-kappa.vercel.app/activity',
      details
    )
